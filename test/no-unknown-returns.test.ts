...
const rule = match(plugin.rules['no-unknown-returns'])
	.with(P.nullish, () => {
		throw new Error("Missing published rule 'no-unknown-returns'.");
	})
...
await test('anti-slop/no-unknown-returns', () => {
	tester.run('anti-slop/no-unknown-returns', rule, {
		valid: [
