...
const rule = match(plugin.rules['no-object-parameters'])
	.with(P.nullish, () => {
		throw new Error("Missing published rule 'no-object-parameters'.");
	})
...
await test('anti-slop/no-object-parameters', () => {
	tester.run('anti-slop/no-object-parameters', rule, {
		valid: [
			'type Alias = object;',
