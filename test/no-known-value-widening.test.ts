...
const rule = match(plugin.rules['no-known-value-widening'])
	.with(P.nullish, () => {
		throw new Error("Missing published rule 'no-known-value-widening'.");
	})
...
await test('anti-slop/no-known-value-widening', () => {
	tester.run('anti-slop/no-known-value-widening', rule, {
		valid: [
