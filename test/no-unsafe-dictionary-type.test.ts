...
const rule = match(plugin.rules['no-unsafe-dictionary-type'])
	.with(P.nullish, () => {
		throw new Error("Missing published rule 'no-unsafe-dictionary-type'.");
	})
...
await test('anti-slop/no-unsafe-dictionary-type', () => {
	tester.run('anti-slop/no-unsafe-dictionary-type', rule, {
		valid: [
